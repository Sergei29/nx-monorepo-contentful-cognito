  ...
  return (
    <section className={clsx('max-w-5xl mx-auto', className)}>
      <ul className="grid grid-cols-gallery gap-2 py-2">
        {games.map(({ id, title, subTitle, image }, index) => (
          <li key={id}>
            <Link
              href={paths.games(id)}
            ...
            >
              ...
              <div className=" bg-black p-2 min-h-16 text-sm">
                <h3 className=" text-yellow-400 font-semibold">{title}</h3>
                <p className="text-xs text-yellow-400"> {subTitle}</p>
              </div>
            </Link>
          </li>
        ))}
